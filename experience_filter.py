        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    # Match to firm
                    firm = match_firm(cleaned_content, thread_title)
                    if not firm:
                        continue
                    ...
                    # Compute quality metrics using cleaned content
                    is_q = is_question(cleaned_content)
                    is_meta = is_meta_low(cleaned_content)
                    is_short = is_too_short(cleaned_content)
                    quality = compute_quality_score(cleaned_content)
...
def main():
    ...
    if args.out and filtered:
        with open(args.out, 'w', newline='', encoding='utf-8') as f:
            # Ensure all fields from processed results are written
            # Dynamically get fieldnames from the first row if available
            if filtered:
                fieldnames = list(filtered[0].keys())
            else:
                # Fallback to a predefined set if no filtered results
                fieldnames = ['firm_name', 'content', 'cleaned_content', 'timestamp', 'thread_url',
